...
class SelfAttention(nn.Module):
    def __init__(self, embed_size, heads):
        ...
        assert (self.head_dim * heads == embed_size), "Embed size is not divisible by heads"
    ...
    def forward(self, values, keys, queries, mask):
        ...
        #Q -> (N, querylen, heads, heads_dim)
        #K-> (N, keylen, heads, heads_dim)
        #out -> (N, heads, querylen, keylen) for every word in our target how much we care attention in our inputs
        
        if mask is not None:
            energy = energy.masked_fill(mask == 0, float("-1e20"))
            
        attention = torch.softmax(energy / (self.embed_size ** (1/2)), dim = 3) #Normalizing across key_len "Source sentence"
...
class SelfAttention_2(nn.Module):
    ...
    def forward(self, values, keys, queries, mask):
        ...
        energy = torch.matmul(queries, keys)  # (N, heads, query_len, key_len)
        ...
        # Reshape back
        out = out.permute(0, 2, 1, 3)  # (N, query_len, heads, head_dim)
...
class TransformerBlock(nn.Module):
    def __init__(self, embed_size, heads, dropout, forward_expansion):
        ...
        self.attention = SelfAttention_2(embed_size, heads)
...
class Encoder(nn.Module):
    def __init__(self, embed_size, heads, dropout, forward_expansion,
                 src_vocab_size, num_layers, device, max_length):
...
class DecoderBlock(nn.Module):
    def __init__(self, embed_size, heads, forward_expanstion, dropout
                 , device):
        super(DecoderBlock, self).__init__()
        self.attention = SelfAttention(embed_size, heads)
        self.norm = nn.LayerNorm(embed_size)
        self.transformer_block = TransformerBlock(
            embed_size, heads, dropout, forward_expanstion
        )
        self.dropout = nn.Dropout(dropout)
...
if __name__ == "__main__":
    ...
    out = model(x, trg[:, :-1])
    predicted_tokens = torch.argmax(out, dim=-1)
